...
const hdrTabPara = (left: string, right: string, spaceBefore = 0) =>
  new Paragraph({
    ...
    children: [
      new TextRun({ text: left, bold: true, boldComplexScript: true, size: 28, color: COLOR, font: 'Times New Roman' }),
      new TextRun({ text: '\t' }),
      new TextRun({ text: right, bold: true, boldComplexScript: true, size: 28, color: COLOR, font: 'Times New Roman' }),
    ],
  });
...
const boldLabel = (text: string) =>
  new Paragraph({ alignment: AlignmentType.JUSTIFIED, spacing: SP, children: [new TextRun({ text, bold: true, font: 'Calibri', size: 22, color: COLOR })] });
      ...
      if (loc) {
        ...
        paras.push(
          new Paragraph({
            alignment: AlignmentType.JUSTIFIED,
            spacing: SP,
            children: [new TextRun({ text: job.roleName ?? 'Role', bold: true, boldComplexScript: true, size: 28, color: COLOR, font: 'Times New Roman' })],
          }),
        );
      }
...
export async function buildPADocx(data: ResumeData): Promise<void> {
  const sectionHdrRun = (t: string) =>
    new TextRun({ text: t, bold: true, size: 28, color: COLOR, font: 'Times New Roman' });
    ...
    sections: [{
      ...
      children: [
        ...
        new Paragraph({
          ...
          children: [new TextRun({ text: data.name ?? 'Full Name', bold: true, size: 36, color: COLOR, font: 'Times New Roman' })],
        }),
        ...
        new Paragraph({
          ...
          spacing: BODY_SPACING,
          children: [
            new TextRun({ text: 'Title/Role:', bold: true, size: 28, color: COLOR, font: 'Times New Roman' }),
            new TextRun({ text: '\t' }),
            new TextRun({ text: 'PeopleFluent Requisition Number:', bold: true, size: 28, color: COLOR, font: 'Times New Roman' }),
          ],
        }),
        new Paragraph({
          tabStops: [RIGHT_TAB],
        ...
        spacer(),
        // Same order as Ohio: Education → Certifications → Employment → Summary → Skills
  ...
  saveAs(blob, `${data.name ?? 'Resume'}_Pennsylvania.docx`);
}
